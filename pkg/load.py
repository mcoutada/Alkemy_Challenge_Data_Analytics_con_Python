...
def load(dfs_dic):
    ...
    # Apply prefix to category names as they will be used as table names in the database
    dfs_dic = {f"alk_{cat}":df for cat, df in dfs_dic.items()}
    
    # function to check if a table exists
    exists_in_db = lambda t: s.inspect(engine).has_table(table_name=t, schema=POSTGRES_SCHEMA)
    ...
    if not all(exist_in_db):
        ...
        if not any(exist_in_db):
            ...
            log.critical("Some tables exist in the database but not all of them. Please check the database.")
            exit()
        

    # Load the dataframes into the database
    for cat, df in dfs_dic.items():
        df.to_sql(name=cat, con=engine, if_exists='replace', index=False, schema=POSTGRES_SCHEMA)
